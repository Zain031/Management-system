...
export const fetchFoodStuffs = createAsyncThunk(
  ...
  async (params, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get('/food-stuffs', { params })
...
export const createFoodStuff = createAsyncThunk(
  ...
  async (foodStuff, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(`/food-stuffs`, foodStuff)
      return response.data
    } catch (e) {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateFoodStuff.fulfilled, (state, action) => {
        const index = state.foodStuffs.findIndex((pkg) => pkg.id === action.payload.id)
        if (index !== -1) {
          state.foodStuffs[index] = action.payload
        }
        state.status = 'succeeded'
      })
      .addCase(updateFoodStuffImage.fulfilled, (state, action) => {
        const index = state.foodStuffs.findIndex((pkg) => pkg.id === action.payload.id)
        if (index !== -1) {
          state.foodStuffs[index] = action.payload
        }
      ...
      .addCase(deleteFoodStuffImage.fulfilled, (state, action) => {
        const index = state.foodStuffs.findIndex((pkg) => pkg.id === action.payload.id)
        if (index !== -1) {
          state.foodStuffs[index] = action.payload
        }
      ...
      .addCase(deleteFoodStuff.fulfilled, (state, action) => {
        state.foodStuffs = state.foodStuffs.filter((pkg) => pkg.id !== action.payload.id)
        state.status = 'succeeded'
      })
      .addMatcher(
