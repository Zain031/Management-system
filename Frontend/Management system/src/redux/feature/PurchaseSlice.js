  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updatePurchase.fulfilled, (state, action) => {
        const index = state.purchases.findIndex((pkg) => pkg.id === action.payload.id)
        if (index !== -1) {
          state.purchases[index] = action.payload
        }
        state.status = 'succeeded'
      })
      .addCase(updatePurchaseImage.fulfilled, (state, action) => {
        const index = state.purchases.findIndex((pkg) => pkg.id === action.payload.id)
        if (index !== -1) {
          state.purchases[index] = action.payload
        }
      ...
      .addCase(deletePurchaseImage.fulfilled, (state, action) => {
        const index = state.purchases.findIndex((pkg) => pkg.id === action.payload.id)
        if (index !== -1) {
          state.purchases[index] = action.payload
        }
      ...
      .addCase(deletePurchase.fulfilled, (state, action) => {
        state.purchases = state.purchases.filter((pkg) => pkg.id !== action.payload.id)
        state.status = 'succeeded'
      })
      .addMatcher(
