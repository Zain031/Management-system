...
export const fetchFoods = createAsyncThunk(
  ...
  async (params, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get('/foods', { params })
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateFood.fulfilled, (state, action) => {
        const index = state.foods.findIndex((pkg) => pkg.id === action.payload.id)
        if (index !== -1) {
      ...
      .addCase(deleteFoodImage.fulfilled, (state, action) => {
        const index = state.foods.findIndex((pkg) => pkg.id === action.payload.id)
        if (index !== -1) {
          state.foods[index] = action.payload
        }
      ...
      .addCase(deleteFood.fulfilled, (state, action) => {
        state.foods = state.foods.filter((pkg) => pkg.id !== action.payload.id)
        state.status = 'succeeded'
      })
      .addMatcher(
